# Distribution of rent % of annual income in all US counties
import pandas as pd
import numpy as np
import requests

# Adding column descriptions
rent_df = pd.read_csv('Census_Rent_Per_Inc.csv')  # Copied from [https://api.census.gov/data/2018/acs/acs5/variables.html] and slightly formatted in excel
rent_df['Vals'] = rent_df['Vals'].apply(lambda x:x.split('!!')[-1:]).apply(lambda x:' '.join(x))
cols = rent_df.Vals.values.tolist()[:-2] + ['Not Computed', 'Median', 'State', 'County']
...
df.to_csv('Census_API_Rent_Per_Inc_17.csv', index=False)